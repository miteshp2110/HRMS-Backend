...
exports.createCycle = async (req, res) => {
  ...
  if (
    ...
    return res.status(400).json({
      message:
        "Cycle name, start date, end date, and at least one group ID are required.",
    });
  }
...
exports.updateCycleStatus = async (req, res) => {
  ...
  try {
    ...
    const [[cycle]] = await connection.query(
      "SELECT * FROM payroll_cycles WHERE id = ? FOR UPDATE",
      [cycleId]
    );
    if (!cycle) {
      await connection.rollback();
    ...
    await connection.query(
      "UPDATE payroll_cycles SET status = ? WHERE id = ?",
      [status, cycleId]
    );
    await connection.commit();
    ...
    res
      ...
      .json({
        success: true,
        message: `Payroll cycle status updated to ${status}.`,
      });
  } catch (error) {
    if (connection) await connection.rollback();
...
exports.deleteCycle = async (req, res) => {
  const { cycleId } = req.params;
  try {
    const [[status]] = await pool.query('select status from payroll_cycles where id = ?',[cycleId])
    if(status.status === 'Paid'){
      return res
        .status(404)
        .json({
          message:
            "Cannot delete Already Paid cycle",
        });
    }
    if(status.status !== 'Draft' && status.status !== 'Auditing'){
      const [payslipsId] = await pool.query('select id from payslips where cycle_id = ?',[cycleId])
    
    payslipsId.forEach(async(payslip)=>{
      const [ppiData] = await pool.query('select item_type,item_id from payslip_processed_items where payslip_id = ?',[payslip.id])
      ppiData.forEach(async(ppi)=>{
        if(ppi.item_type === 'hr_case'){
          await pool.query(`update hr_cases set status = 'Approved' , payslip_id = null where id = ?`,[ppi.item_id])
        }
        else if(ppi.item_type === 'loan_emi'){
          
          await pool.query(`update loan_amortization_schedule set status = 'Pending'  where id = ?`,[ppi.item_id])
        }
        else if(ppi.item_type === 'expense_claim'){
          await pool.query(`update expense_claims set status = 'Processed' , reimbursed_in_payroll_id = null where id = ?`,[ppi.item_id])
        }
      })
    })
    }
    ...
    const [result] = await pool.query(
      `DELETE FROM payroll_cycles WHERE id = ? AND status != 'Paid'`,
      [cycleId]
    );
    if (result.affectedRows === 0) {
      return res
        ...
        .json({
          message:
            "Draft payroll cycle not found or it has already been processed.",
        });
    }
    res.status(204).send();
  } catch (error) {
...
exports.executeGroupRun = async (req, res) => {
  ...
  // console.log("group run executing")
  ...
  try {
    ...
    const [[cycle]] = await connection.query(
      "SELECT * FROM payroll_cycles WHERE id = ? AND status IN ('Auditing', 'Review')",
      [cycleId]
    );
    if (!cycle) {
      return res
        .status(404)
        .json({ message: "Active payroll cycle not found." });
    }
    ...
    const [cids] = await connection.query(
      "select component_id as cid from payroll_group_components where group_id = ?",
      [groupId]
    );
    cids.forEach((c) => {
      if (c.cid === 97) {
    ...
    if (components.length === 0) {
      ...
      return res
        .status(400)
        .json({ message: "No components found for this group." });
    }
    ...
    for (const employee of employees) {
      try {
        const { earnings, deductions, processedItems } =
          await calculateEmployeePayslip(
            connection,
            employee.id,
            components,
            cycle
        ...
        if (earnings.length > 0) {
            ...
            // console.log("insert earning: ",earningValues)
        ...
        if (deductions.length > 0) {
            ...
            // console.log("deduction values: ",deductionValues)
            await connection.query(
                'INSERT INTO payslip_details (payslip_id, component_id, component_name, component_type, amount, calculation_breakdown) VALUES ?',
                [deductionValues]
            );
        ...
        if (processedItems.length > 0) {
          ...
          await connection.query(
            "DELETE FROM payslip_processed_items WHERE payslip_id = ?",
            [payslipId]
          );

          const processedValues = processedItems.map((item) => [
            payslipId,
            item.item_type,
            item.item_id,
            "Processed",
          ...
          for (const item of processedValues) {
            if (item[1] === "loan_emi") {
              await connection.query(
                "UPDATE loan_amortization_schedule SET status = 'Locked' WHERE id = ?",
                [item[2]]
              );
            } else if (item[1] === "hr_case") {
              await connection.query(
                "UPDATE hr_cases SET status = 'Locked', payslip_id = ?  WHERE id = ?",
                [item[0],item[2]]
              );
            } else if (item[1] === "expense_claim") {
              await connection.query(
                "UPDATE expense_claims SET reimbursed_in_payroll_id = ?, status = 'Locked' WHERE id = ?",
                [item[0], item[2]]
              );
            }
          }
...
async function generatePayslipsForCycle(connection, cycle) {
    ...
    console.log("another function runing")
  ...
  const [cids] = await connection.query(
    "select component_id as cid from payroll_group_components where group_id = (select group_id from payroll_cycle_groups where cycle_id = ?)",
    [cycle.id]
  );
  cids.forEach((c) => {
    if (c.cid === 97) {
  ...
  for (const employee of employees) {
    try {
      ...
      if (existingPayslip) {
        payslipId = existingPayslip.id;
        await connection.query(
          "DELETE FROM payslip_details WHERE payslip_id = ?",
          [payslipId]
        );
        await connection.query(
          "DELETE FROM payslip_processed_items WHERE payslip_id = ?",
...
async function finalizeProcessedItems(connection, cycleId) {
  ...
  for (const loan of loansToFinalize) {
    ...
    await connection.query(
      "UPDATE payslip_processed_items SET status = 'Finalized' WHERE id = ?",
      [loan.ppi_id]
    );
  }
...
exports.markAsPaid = async (req,res)=>{
  const{cycleId,jv} = req.body
  const connection = await pool.getConnection() 
  try{
    if(!cycleId || !jv){
      res.status(404).json({message:"Please provide Jv number and cycleId"})
    }
    ...
    payslipsId.forEach(async(payslip)=>{
      ...
      ppiData.forEach(async(ppi)=>{
        if(ppi.item_type === 'hr_case'){
          await pool.query(`update hr_cases set status = 'Closed' where id = ?`,[ppi.item_id])
          await pool.query(`update payslip_processed_items set status = 'Finalized' where id = ?`,[ppi.id])
        }
        else if(ppi.item_type === 'loan_emi'){
          const [[las]] = await pool.query(`select loan_application_id,id as schedule_id,emi_amount from loan_amortization_schedule where id = ?`,[ppi.item_id])

          await pool.query(`insert into loan_repayments (loan_application_id,schedule_id,payslip_id,repayment_amount,repayment_date) values (?,?,?,?,UTC_TIMESTAMP())`,[las.loan_application_id,las.schedule_id,payslip.id,las.emi_amount])
          await pool.query(`update loan_amortization_schedule set status = 'Paid'  where id = ?`,[ppi.item_id])
          await pool.query(`update payslip_processed_items set status = 'Finalized' where id = ?`,[ppi.id])
        }
        else if(ppi.item_type === 'expense_claim'){
          await pool.query(`update expense_claims set status = 'Reimbursed' where id = ?`,[ppi.item_id])
          await pool.query(`update payslip_processed_items set status = 'Finalized' where id = ?`,[ppi.id])
        }
      })
    })
    await pool.query(`update payroll_cycles set jv_number = ? , status = 'Paid' where id = ?`,[jv,cycleId])
    res.status(200).json({message:"Marked Cycle as Paid"})

  }
  catch (error) {
    if (connection) await connection.rollback();
    console.error("Error While Marking as Paid", error);
    res.status(500).json({ message: "Error while marking as paid" });
  } finally {
    if (connection) connection.release();
  }
}