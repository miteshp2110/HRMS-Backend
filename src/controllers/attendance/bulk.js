...
//     const { reason, attendance_date, punch_in_local, punch_out_local, timezone, records, is_late = false, is_early_departure = false,status } = req.body;
...
const bulkCreateAttendance = async (req, res) => {
    ...
    if (!status && !punch_in_local && !punch_out_local) {
        return res.status(400).json({ message: 'At least one of punch_in_local or punch_out_local must be provided if status is not Absent or Leave.' });
    }
    ...
    try {
        ...
        for (const record of records) {
            ...
            if (status === 'Absent' || status === 'Leave') {
                ...
                `;
                await connection.query(upsertSql, [employee_id, attendance_date, user.shift, shift.scheduled_hours, status, changed_by]);
            ...
            if (punch_in_local && !punch_out_local) {
                ...
                const punchInTime = DateTime.fromJSDate(existingRecord.punch_in, { zone: 'utc' });
                const hours_worked = isHoliday ? 0 : calculateHoursWorked(punchInTime, effectivePunchOutTime);
                const short_hours = isHoliday ? 0 : Math.max(0, shift.scheduled_hours - hours_worked);
                let attendance_status = (isHoliday || hours_worked >= shift.half_day_threshold) ? 'Present' : 'Half-Day';
